import { createRootRoute, Link, Outlet } from '@tanstack/react-router'
import { TanStackRouterDevtools } from '@tanstack/router-devtools'
import './__root.scss'
import React from "react";

export const Route = createRootRoute({
    component: () => (
        <div className="app">
            <header className='header'>
                <Link to="/">
                    <img src='src/assets/logo.svg' alt='Логотип' className='header__logo'/>
                </Link>
                <nav className="header__nav">
                    <ul className="nav__list">
                        <li className="nav__item">
                            <Link to='/'>
                                Уведомления
                            </Link>
                        </li>
                        <li className="nav__item">
                            <Link to='/documents'>
                                Документы
                            </Link>
                        </li>
                        <li className="nav__item">
                            <Link to='/'>
                                Заявления
                            </Link>
                        </li>
                        <li className="nav__item">
                            <Link to='/'>
                                Платежи
                            </Link>
                        </li>
                        <li className="nav__item">
                            <Link to='/'>
                                Помощь
                            </Link>
                        </li>
                        <li className="nav__item">
                            <div className="nav__search-login">
                                <img src='src/assets/Search.svg' alt='Поиск' className='search'/>
                                <span className="login__link">
                                Войти
                            </span>
                            </div>
                        </li>
                    </ul>
                </nav>
            </header>
            
            <Outlet />
            <TanStackRouterDevtools />
            
           <div className='footer'>
            <div className='footer_info'>
            <ul>
                <li>Личный кабинет</li>
                <li>Как найти услугу</li>
                <li>Регистрация на Госуслугах</li>
            </ul>
            <ul>
                <li>Бизнесу</li>
                <li>Иностранцам</li>
                <li>Партнерам</li>
            </ul>
            <ul>
                <li>Помощь</li>
                <li>Контакты</li>
                <li>Карта центров обслуживания</li>
            </ul>
            <ul className='social'>
                <li>
                    <svg 
                        width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0 19.2C0 12.4794 0 9.11905 1.30792 6.55211C2.4584 4.29417 4.29417 2.4584 6.55211 1.30792C9.11905 0 12.4794 0 19.2 0H20.8C27.5206 0 30.8809 0 33.4479 1.30792C35.7058 2.4584 37.5416 4.29417 38.6921 6.55211C40 9.11905 40 12.4794 40 19.2V20.8C40 27.5206 40 30.8809 38.6921 33.4479C37.5416 35.7058 35.7058 37.5416 33.4479 38.6921C30.8809 40 27.5206 40 20.8 40H19.2C12.4794 40 9.11905 40 6.55211 38.6921C4.29417 37.5416 2.4584 35.7058 1.30792 33.4479C0 30.8809 0 27.5206 0 20.8V19.2Z" fill="#86909C"/>
                        <path d="M26.6834 28.2398L29.9809 11.6281C29.9809 10.522 28.46 11.1926 28.467 11.1926L9.96112 18.4511C9.96112 18.4511 9.08319 18.7484 9.15232 19.3014C9.22144 19.8545 9.93347 20.1102 9.93347 20.1102L14.5859 21.6795L14.5951 21.6839C14.5942 21.681 14.5938 21.6795 14.5938 21.6795L25.7719 14.58C25.7719 14.58 26.4148 14.1859 26.3941 14.58C26.3941 14.58 26.5116 14.6491 26.1659 14.974C25.8203 15.2989 17.3866 22.8755 17.3866 22.8755L17.3686 23.0271L24.9552 28.8412C26.2203 29.3943 26.6834 28.2398 26.6834 28.2398Z" fill="white"/>
                    </svg>
                </li>
                <li>
                    <svg
                        width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0 19.2C0 12.4794 0 9.11905 1.30792 6.55211C2.4584 4.29417 4.29417 2.4584 6.55211 1.30792C9.11905 0 12.4794 0 19.2 0H20.8C27.5206 0 30.8809 0 33.4479 1.30792C35.7058 2.4584 37.5416 4.29417 38.6921 6.55211C40 9.11905 40 12.4794 40 19.2V20.8C40 27.5206 40 30.8809 38.6921 33.4479C37.5416 35.7058 35.7058 37.5416 33.4479 38.6921C30.8809 40 27.5206 40 20.8 40H19.2C12.4794 40 9.11905 40 6.55211 38.6921C4.29417 37.5416 2.4584 35.7058 1.30792 33.4479C0 30.8809 0 27.5206 0 20.8V19.2Z" fill="#86909C"/>
                        <path d="M21.2833 28.816C12.1666 28.816 6.96667 22.566 6.75 12.166H11.3167C11.4667 19.7993 14.8333 23.0327 17.4999 23.6993V12.166H21.8V18.7493C24.4334 18.466 27.1997 15.466 28.1331 12.166H32.4332C31.7165 16.2327 28.7165 19.2326 26.5832 20.466C28.7165 21.466 32.1334 24.0827 33.4333 28.816H28.6999C27.6832 25.6493 25.15 23.1993 21.8 22.866V28.816H21.2833Z" fill="white"/>
                    </svg>
                </li>
                <li>
                    <svg 
                        width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0 19.2C0 12.4794 0 9.11905 1.30792 6.55211C2.4584 4.29417 4.29417 2.4584 6.55211 1.30792C9.11905 0 12.4794 0 19.2 0H20.8C27.5206 0 30.8809 0 33.4479 1.30792C35.7058 2.4584 37.5416 4.29417 38.6921 6.55211C40 9.11905 40 12.4794 40 19.2V20.8C40 27.5206 40 30.8809 38.6921 33.4479C37.5416 35.7058 35.7058 37.5416 33.4479 38.6921C30.8809 40 27.5206 40 20.8 40H19.2C12.4794 40 9.11905 40 6.55211 38.6921C4.29417 37.5416 2.4584 35.7058 1.30792 33.4479C0 30.8809 0 27.5206 0 20.8V19.2Z" fill="#86909C"/>
                        <path d="M20 20C21.3833 20 22.6333 19.4333 23.5333 18.5333C24.4333 17.6333 25 16.3833 25 15C25 13.6167 24.4333 12.3667 23.5333 11.4667C22.6333 10.5667 21.3833 10 20 10C18.6167 10 17.3667 10.5667 16.4667 11.4667C15.5667 12.3667 15 13.6167 15 15C15 16.3833 15.5667 17.6333 16.4667 18.5333C17.3667 19.4333 18.6333 20 20 20ZM18.4 13.3833C18.8167 12.9667 19.3833 12.7167 20 12.7167C20.6333 12.7167 21.2 12.9667 21.6 13.3833C22.0167 13.8 22.2667 14.3667 22.2667 14.9833C22.2667 15.6167 22.0167 16.1833 21.6 16.5833C21.1833 17 20.6167 17.25 20 17.25C19.3667 17.25 18.8 17 18.4 16.5833C17.9833 16.1667 17.7333 15.6 17.7333 14.9833C17.7333 14.3667 18 13.8 18.4 13.3833Z" fill="white"/>
                        <path d="M26.9474 22.0447L25.433 20.0964C25.347 19.9828 25.1749 19.9666 25.0716 20.064C23.6433 21.2167 21.8879 21.9635 19.9949 21.9635C18.1019 21.9635 16.3466 21.2167 14.9183 20.064C14.815 19.9828 14.6429 19.999 14.5569 20.0964L13.0425 22.0447C12.9736 22.1421 12.9908 22.272 13.0769 22.3531C14.3504 23.3273 15.8303 23.9929 17.3792 24.3339L14.1783 29.6591C14.0922 29.8052 14.2127 30 14.3848 30H17.5168C17.6201 30 17.7061 29.9513 17.7406 29.8539L19.9777 25.1456L22.2149 29.8539C22.2493 29.9351 22.3354 30 22.4386 30H25.5707C25.76 30 25.8632 29.8214 25.7772 29.6591L22.5763 24.3339C24.1251 23.9929 25.6051 23.3435 26.8786 22.3531C27.0162 22.272 27.0335 22.1421 26.9474 22.0447Z" fill="white"/>
                    </svg>
                </li>
                <li>
                    <svg 
                        width="40" height="40" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M0 19.2C0 12.4794 0 9.11905 1.30792 6.55211C2.4584 4.29417 4.29417 2.4584 6.55211 1.30792C9.11905 0 12.4794 0 19.2 0H20.8C27.5206 0 30.8809 0 33.4479 1.30792C35.7058 2.4584 37.5416 4.29417 38.6921 6.55211C40 9.11905 40 12.4794 40 19.2V20.8C40 27.5206 40 30.8809 38.6921 33.4479C37.5416 35.7058 35.7058 37.5416 33.4479 38.6921C30.8809 40 27.5206 40 20.8 40H19.2C12.4794 40 9.11905 40 6.55211 38.6921C4.29417 37.5416 2.4584 35.7058 1.30792 33.4479C0 30.8809 0 27.5206 0 20.8V19.2Z" fill="#86909C"/>
                        <path d="M25.4 21.6V21.8L29.5 29.6H24.5L21.4 22.9V22.7H16.3V29.6H12V10H21.7C25.1 10 27.7 12.6 27.7 16V16.9C27.8 18.9 26.8 20.4 25.4 21.6ZM16.5 19H20.4C21.1 19 21.8 18.8 22.2 18.4C22.7 18 23 17.4 23 16.7V16.5C23 15 21.9 14 20.4 14H16.5V19Z" fill="white"/>
                        <path d="M15 22H22V23H15V22Z" fill="white"/>
                    </svg>
                </li>
            </ul>
        </div>
    </div>
</div>
    ),
})